...
import WinOrLoss from '../GameOver'
import './index.css'
...
class MatchGame extends Component {
  state = {
    ...
    intialList: imagesList,
    Score: 0,
    selectTab: tabsList[0].tabId,
    intialImageId: imagesList[0].id,
  ...
  tick = () => {
    this.setState(perState => {
      return {timeSec: perState.timeSec - 1}
    })
  }
  ...
  getFilteredProjects =()=>{
    ...
    const filteredProjects = imagesList.filter(each => each.category === selectTab)
  ...
  onImageClick = id => {
    const {displayImg, intialImageId, timeSec} = this.state
    if (intialImageId === id && timeSec > 0) {
      const flipedList = this.getShuffledEmojisList()

      this.setState(perState => ({
        intialList: perState.intialList,
        Score: perState.Score + 1,
        intialImageId: flipedList[0].id,
        displayImg: flipedList[0].imageUrl,
      }))
    } else {
      clearInterval(this.TimerId)
  ...
  render() {
    const {intialList, selectTab, displayImg, Score, GameOver, timeSec} =
      this.state
    const FilterImgList = this.getFilteredProjects()  

    return (
      <ul type="none">
        <li className="navBarCard">
          <img
          ...
          <div className="scoreAndTImercard">
            ...
            <img
              ...
              className="timerImg"
            />
            <p className="scorePara">{timeSec} Sec</p>
          </div>
        </li>

        <li>
          {GameOver ? (
            <WinOrLoss Score={Score} Replay={this.Replay} />
          ) : (
            <li className="ImgViewContainer">
              <img src={displayImg} alt="match" />
              <div>
                <ul type="none" className="TabContainer">
                  {tabsList.map(each => (
                    <Tabs
                      TabDetails={each}
                      TabId={each.tabId}
                      OnTabClick={this.OnTabClick}
                      isFav={this.selectTab}
                      key={each.tabId}
                      onActive={each.tabId === selectTab}
                    />
                  ))}
                </ul>
              </div>
              <div>
                <ul type="none" className="thumbNailContainer">
                  {FilterImgList.map(eachImg => (
                    <TabItems
